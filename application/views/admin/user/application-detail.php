            ...
            <div class="card">
               <form action="<?php echo base_url('documents/update');?>" method="post">
                  <div class="card-body">
                     ...
                     <button type="submit" name="submit" id="update-app" class="btn btn-primary mt-3">Update Application</button>
               </form>
               </div>
            </div>
         </div>
...
<script>
    document.getElementById("update-app").addEventListener("click", function(event) {
        var checkboxes = document.querySelectorAll('input[name="documents[]"]:checked');
        if (checkboxes.length === 0) {
            jQuery('.error_Msg').text("Please select at least one document.");
