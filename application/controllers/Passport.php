...
class Passport extends Front_Controller{
	...
	public function index()
	{
		...
		public function visa_page()
	{
	
		$data['main_content'] = 'visa';
		$this->load->view('front/inc/view',$data);
	}
...
// Load the FedEx configuration file
	    ...
	    $data['visa_statuses'] =  $this->input->post('has_visa');
	    	$data['countries'] =   $this->input->post('countries_name');
	    	 $selectedValuesEncoded = $this->input->post('selected_values');
	    	  $selectedValues = html_entity_decode($selectedValuesEncoded);
	    	   $selectedValuesArray = json_decode($selectedValues, true);
		$data['visa_required_results'] = $selectedValuesArray;
	...
	public function  visa_ask_lead()
	{
		...
		if(isset($_POST)){
		 ...
		 $countries_assistance = isset($_POST['countries_assistance']) ? $_POST['countries_assistance'] : '';
         $countries_assistance_json = json_encode($countries_assistance);
           ...
           $data = array(
            ...
            'requirement' => $countries_assistance_json
        );
	...
	public function  request_consultation()
	{
		...
		if(isset($_POST)){
			...
			$config['smtp_pass']    = "qnah zrvs ekqq lkro";
		...
		public function  visa_consultation()
	{
		...
		if(isset($_POST)){
			...
			$section['subject'] = 'Request Consultation';
			$body = $this->load->view('front/email/visa_consultation', $section, TRUE);
...
// 	}
	...
	public function process_payment() {
        ...
        foreach ($cart_items as $index => $cart_item) {
			...
			$expirationyear = $this->input->post('cardExpYear');
			$expirationmonth = $this->input->post('cardExpMonth');

			$expiration_date = $expirationyear . '-' . str_pad($expirationmonth, 2, '0', STR_PAD_LEFT);
			$cvv =  $this->input->post('ccode');
			$rep_data = $this->session->userdata('recipient_address');
			...
			$recipient= array(
				...
				'zipcode' => $rep_data['zipcode'],
				'person_name' => $rep_data['person_name'][0],
				'contact_number' => $rep_data['contact_number'][0],
				'rate_type' => $shipping_type,
			);
			...
			$shippingLabelGenerator = new Shipping_label();
			$labelFromOriginToRecipient = $shippingLabelGenerator->generateLabelFromOriginToRecipient($origin, $recipient, $shipping_type);
			$labelFromRecipientToOrigin = $shippingLabelGenerator->generateLabelFromRecipientToOrigin($recipient, $origin, $shipping_type);
			$labels = array(
				'origin' => $labelFromOriginToRecipient ,
				'recipient' => $labelFromRecipientToOrigin
			);
			...
			if (($response != null) && ($response->getMessages()->getResultCode() == "Ok")) {
				...
				$transaction_status =  $transactionResponse->getMessages()[0]->getDescription();

				$reason = "Response Code: " . $response_code . ", Message Code: " . $message_code . ", Transaction Status: " . $transaction_status;
				...
				$transaction_data = array(
					...
					'payment_response' => $reason,
					'fedex_shipping_fee'=>$fee,
					'shipping_type'=>$shipping_type,
					'shipping_label'=>$labels['origin']['filename'],
					'auth_code'=>$trannaction_id,
				...
				if (!empty($transaction_id)) {
					...
					if (!empty($customer_id)) {
						...
						<td bgcolor="#ffffff" align="left" style="padding: 50px 30px 50px 30px;">
							...
							<p style="font-family: \'lato\'; font-size: 18px; font-weight: 400; color: #000;">Thank you for previously placing your order with RJR Passports!</p> <br><p style="font-family: \'lato\'; font-size: 18px; font-weight: 400; color: #000;">Your tracking number is: '.$labels['origin']['trackingNumber'].' </p>';
    ...
    <div class="invoice-container">
        ...
        <div class="invoice-footer">
			...
			$section['subject'] = 'Document Requirements';
						...
						$this->email->subject($subject);
						$this->email->message($body);
						$this->email->attach($labels['recipient']['pdfFilePath']);
						$mails = $this->email->send();
						if ($mails) {
							$link = base_url('document-requirements');
							$this->load->library('email');
